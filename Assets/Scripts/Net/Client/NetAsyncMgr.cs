...
namespace AE_NetWork
{
    public class NetAsyncMgr : MonoBehaviour
    {
        ...
        //消息队列
        private Queue<BaseHandler> reciveMessageQueue = new Queue<BaseHandler>();

        private static readonly float HeartMessageIntervalTime = 2f;
        ...
        private void Update()
        {
            ...
            {
                BaseHandler handler = reciveMessageQueue.Dequeue();
                if (handler != null)
                {
                    handler.Handle();
                }
                ...
                while (true)//粘包
                {
                    ...
                    if (bufferLenght - currentIndex >= messageBodyLength && massageID != -1)
                    {
                        ...
                        if (baseMassage != null)
                        {
                            ...
                            reciveMessageQueue.Enqueue(handler);
                        }
